...
const FormContacto = ({ onNextStep, data }) => {
  ...
  const allowedLetters = ['B', 'C', 'D', 'F', 'G', 'H', 'J', 'K', 'L', 'P', 'R', 'S', 'T', 'V', 'W', 'X', 'Y', 'Z'];
  ...
  const formatPatent = (value) => {
    ...
    cleanValue = cleanValue.split('').filter(char => allowedLetters.includes(char) || !isNaN(char)).join('');
    ...
    if (cleanValue.length <= 6 && /^[A-Z]{2}/.test(cleanValue)) {
      ...
      return cleanValue.replace(/^([A-Z]{2})([A-Z]{2})([0-9]{0,2})$/, '$1-$2·$3');
    }
    ...
    <div>
      ...
      <form ref={formRef} onSubmit={handleSubmit}>
        ...
        <div className="formControl-root">
          ...
          <div className='outlinedInput-root textField-root inputBase-root'>
            <input type="email" onChange={handleInputChange} value={formData.email} name="email" placeholder="mail@mail.com" className='form-input' />
          </div>
        </div>
        <div className="formControl-root">
          ...
          <div className='outlinedInput-root textField-root inputBase-root'>
            <input onChange={handleInputChange} value={formData.registrationNumber} className='form-input' type="text" name="registrationNumber" />
          </div>
        </div>
